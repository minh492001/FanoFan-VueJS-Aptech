  ...
  <div id="loading-container w-100 h-100">
    ...
    <div v-else>
      <div v-if="!searchFanAttribute && !nameOfBrand && !orderByType">
        <BrandsHeader @changeBrand="getTableFans"></BrandsHeader>
        ...
        <hr>
        <RecentlyView :recentList="tableFansList"></RecentlyView>
        <hr>
        <div class="container-fluid">
          <div class="title ms-5">
            <h2>Table Fans</h2>
            <p>Shop table fan from many brands over the world and more</p>
          </div>
          <FanRenderLayout :FansList="tableFansList"></FanRenderLayout>
        </div>
        <hr>
      </div>
      <div v-else>
        ...
        <div v-else-if="nameOfBrand || orderByType || searchFanAttribute">
          ...
          <div class="container-fluid">
            <div v-if="!searchFanAttribute">
              <div class="title ms-5">
                <h2 class="mt-3 mb-4">( {{ tableFansList.length }} ) results founded...</h2>
              </div>
              <FanRenderLayout :FansList="tableFansList"></FanRenderLayout>
            </div>
            <div v-else>
...
const tableFansList = computed(() => store.state.tableFans.data.tableFan);
const feature = ref({})

function getTableFans(brandName) {
  nameOfBrand.value = brandName
  store.dispatch('getTableF', {
...
async function featureProduct() {
  return await axiosClient.get(process.env.VUE_APP_API_URL + 'fans/feature/32').then((res) => {
...
const getArticle = async () => {
  ...
  await axiosInstance.get(process.env.VUE_APP_API_URL + "article/table").then((res) => {
