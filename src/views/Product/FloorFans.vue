  ...
  <div id="loading-container w-100 h-100">
    ...
    <div v-else>
      <div v-if="!searchFanAttribute && !nameOfBrand && !orderByType">
        <BrandsHeader @changeBrand="getFloorFans"></BrandsHeader>
        ...
        <div class="container-feature-product m-auto" v-for="(item,index) in feature" :key="index">
          ...
          <div class="feature-row row bg-white p-0 mt-3 me-0">
            ...
            <div class="ads-video-img p-0">
              <img class="w-50 h-100" :src="item.images[0].image" alt="">
            </div>
            <div class="ads-video">
        ...
        <hr>
        <RecentlyView :recentList="floorFansList"></RecentlyView>
        <hr>
        <div class="container-fluid">
          <div class="title ms-5">
            <h2>Pedestral Fans</h2>
            <p>Shop floor - pedestral fan from many brands over the world and more</p>
          </div>
          <FanRenderLayout :FansList="floorFansList"></FanRenderLayout>
        </div>
        <hr>
      </div>
      <div v-else>
        ...
        <div v-else-if="nameOfBrand || orderByType || searchFanAttribute">
          ...
          <div class="container-fluid">
            <div v-if="!searchFanAttribute">
              <div class="title ms-5">
                <h2 class="mt-3 mb-4">( {{ floorFansList.length }} ) results founded...</h2>
              </div>
              <FanRenderLayout :FansList="floorFansList"></FanRenderLayout>
            </div>
            <div v-else>
...
const floorFansList = computed(() => store.state.floorFans.data.floorFan);
const feature = ref({})

function getFloorFans(brandName) {
  nameOfBrand.value = brandName
  store.dispatch('getFloorF', {
...
async function featureProduct() {
  return await axiosClient.get(process.env.VUE_APP_API_URL + 'fans/feature/122').then((res) => {
    console.log(res)
...
const getArticle = async () => {
  ...
  await axiosInstance.get(process.env.VUE_APP_API_URL + "article/floor").then((res) => {
